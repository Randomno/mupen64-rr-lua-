...
LRESULT CALLBACK PlayMovieProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDC_MOVIE_BROWSE:
		{
			...
			Config.recent_movie_paths[0] = std::string(default_file_dialog_path);
	...
	case WM_COMMAND:
		...
		case IDOK:
		{
			...
			if (flag == MOVIE_START_FROM_EXISTING_SNAPSHOT) {
				...
				GetDefaultFileDialogPath(tempbuf, states_path);
				Config.states_path = std::string(states_path);
		...
		case IDC_MOVIE_BROWSE:
		{
			...
			char default_file_dialog_path[MAX_PATH];
			GetDefaultFileDialogPath(tempbuf, default_file_dialog_path);
			Config.recent_movie_paths[0] = std::string(default_file_dialog_path);
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam)
{
		...
		if (manualFPSLimit && !hotkeys.empty())
		{
			if ((int)wParam == Config.fast_forward_hotkey.key) // fast-forward on
			{
				if (((GetKeyState(VK_SHIFT) & 0x8000) ? 1 : 0) == hotkeys[0]->shift
					...
					&& ((GetKeyState(VK_MENU) & 0x8000) ? 1 : 0) == hotkeys[0]->alt)
				{
					manualFPSLimit = 0;
					hit = TRUE;
				}
	...
	case WM_SYSKEYUP:
	case WM_KEYUP:
		if ((int)wParam == hotkeys[0]->key) // fast-forward off
		{
			manualFPSLimit = 1;
	...
	case WM_COMMAND:
	{
		...
		case IDLOAD:
		{
			...
			char default_file_dialog_path[MAX_PATH];
			GetDefaultFileDialogPath(path_buffer, default_file_dialog_path);
			Config.recent_rom_paths[0] = std::string(default_file_dialog_path);
		...
		case STATE_SAVEAS:
		{
			...
			char default_file_dialog_path[MAX_PATH];
			GetDefaultFileDialogPath(path_buffer, default_file_dialog_path);
			Config.states_path = std::string(default_file_dialog_path);
...
//                        MessageBox(NULL, "Couldn't stop playback.", "VCR", MB_OK);
		...
		case GENERATE_BITMAP: // take/capture a screenshot
			...
			if (Config.is_default_screenshots_directory_used) {
				...
				Config.screenshots_directory.copy(directory, MAX_PATH);
	...
	HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	else
	{
		...
		if (!hwnd)
		{
			MessageBox(NULL, "Failed to create the main window", NULL, MB_ICONERROR);
			return 0;
		}
		...
		// WS_EX_LAYERED fixes dc copying when offscreen
		...
		while (GetMessage(&Msg, NULL, 0, 0) > 0)
		{
			...
			{
				if ((GetKeyState(hotkeys[i]->key) & 0x8000) == 0)
				{
					continue;
				}
				...
				if (i != 0)
				{
					if (((GetKeyState(VK_SHIFT) & 0x8000) ? 1 : 0) == hotkeys[i]->shift
						...
						&& ((GetKeyState(VK_MENU) & 0x8000) ? 1 : 0) == hotkeys[i]->alt)
					{
						// this approach puts us at the mercy of hid drivers and winapi :(
						// we need to compensate for possible duplicate GetKeyState values
						if (last_pressed_hotkey == hotkeys[i])
						{
							last_pressed_hotkey = 0;
							break;
						}
