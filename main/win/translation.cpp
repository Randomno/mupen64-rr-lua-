...
{    
    ...
    // TODO: implement this properly without winbase calls
}
	...
	while ( p ) 
    {
		if (strcmp(p->language_name,"English") == 0) { 
		    if (Config.language == "English") {
		        ...
		        if (Config.language == std::string(p->language_name)) {
...
{
    // TODO: remove current translation system in place for winapi stringtable
    snprintf(Ret, MAX_PATH, GuiWord);
   /*if (current_lang) {
...
static void SetHotkeyMenuAccelerators(t_hotkey* hotkeys, HMENU hmenu, int menuItemID)
{
    char buf [64];
	extern void hotkey_to_string(t_hotkey * hotkey, char* string);
